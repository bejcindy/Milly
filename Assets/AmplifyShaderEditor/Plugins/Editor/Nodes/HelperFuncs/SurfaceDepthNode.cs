            ...
            bool ignoreLocalvar)
        {
            if (dataCollector.IsTemplate)
            {
                if (m_inputPorts[0].IsConnected)
                {
                    ...
                    dataCollector.TemplateDataCollectorInstance.RegisterCustomInterpolatedData(eyeVarName,
                        WirePortDataType.FLOAT, CurrentPrecisionType, instruction);
                    return eyeVarName;
                }
                else
                {
                    return dataCollector.TemplateDataCollectorInstance.GetEyeDepth(CurrentPrecisionType, true,
                        MasterNodePortCategory.Fragment, m_viewSpaceInt);
                }
            }
                ...
                dataCollector.PortCategory == MasterNodePortCategory.Tessellation)
            {
                ...
                dataCollector.AddVertexInstruction(
                    "float " + m_vertexNameStr[m_viewSpaceInt] + " = " + vertexInstruction, UniqueId);
            ...
            if (dataCollector.TesselationActive)
            {
                if (m_inputPorts[0].IsConnected)
                {
                    ...
                    RegisterLocalVariable(0, string.Format("-UnityObjectToViewPos( {0} ).z", value) + space,
                        ref dataCollector, "customSurfaceDepth" + OutputId);
                    return m_outputPorts[0].LocalValue(dataCollector.PortCategory);
                }
                else
                {
                    string eyeDepth =
                        GeneratorUtils.GenerateScreenDepthOnFrag(ref dataCollector, UniqueId, CurrentPrecisionType);
                    if (m_viewSpaceInt == 1)
                    {
                        dataCollector.AddLocalVariable(UniqueId, CurrentPrecisionType, WirePortDataType.FLOAT,
                            m_vertexNameStr[1], eyeDepth + " * _ProjectionParams.w");
            ...
            else
            {
                ...
                if (m_inputPorts[0].IsConnected)
                {
                    ...
                    string instruction = "-UnityObjectToViewPos( " + varName + " ).z" + space;
                    dataCollector.AddToVertexLocalVariables(UniqueId,
                        Constants.VertexShaderOutputStr + "." + varName + " = " + instruction + ";");
                ...
                else
                {
                    ...
                    string instruction = "-UnityObjectToViewPos( " + Constants.VertexShaderInputStr +
                                         ".vertex.xyz ).z" + space;
                    dataCollector.AddToVertexLocalVariables(UniqueId,
                        Constants.VertexShaderOutputStr + "." + m_vertexNameStr[m_viewSpaceInt] + " = " + instruction +
                        ";");
