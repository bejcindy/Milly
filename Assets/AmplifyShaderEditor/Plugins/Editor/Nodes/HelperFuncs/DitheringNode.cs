...
namespace AmplifyShaderEditor
{
    ...
    public sealed class DitheringNode : ParentNode
    {
        ...
        [SerializeField] private int m_selectedPatternInt = 0;

        [SerializeField] private bool m_customScreenPos = false;
            ...
            string name, WirePortDataType type)
        {
            ...
            if (!m_texPort.CheckValidType(type))
            {
                ...
                UIUtils.ShowMessage(UniqueId,
                    "Dithering node only accepts SAMPLER2D input type.\nTexture Object connected changed to " + type +
                    ", connection was lost, please review and update accordingly.", MessageSeverity.Warning);
            }
        }
            ...
            switch (m_selectedPatternInt)
            {
                ...
                case 0:
                {
                    ...
                    IOUtils.AddFunctionHeader(ref m_functionBody,
                        "inline float Dither" + PatternsFuncStr[m_selectedPatternInt] + "( int x, int y )");
                ...
                case 2:
                {
                    ...
                    IOUtils.AddFunctionHeader(ref m_functionBody,
                        "inline float Dither" + PatternsFuncStr[m_selectedPatternInt] + "( float4 screenPos, " +
                        GeneratorUtils.GetPropertyDeclaraction("noiseTexture", TextureType.Texture2D, ", ") +
                        GeneratorUtils.GetSamplerDeclaraction("samplernoiseTexture", TextureType.Texture2D, ", ") +
                        "float4 noiseTexelSize )");

                    string samplingCall = GeneratorUtils.GenerateSamplingCall(ref dataCollector,
                        WirePortDataType.SAMPLER2D, "noiseTexture", "samplernoiseTexture",
                        "screenPos.xy * _ScreenParams.xy * noiseTexelSize.xy", MipType.MipLevel, "0");
            ...
            bool ignoreLocalVar)
        {
            ...
            if (m_customScreenPos && m_inputPorts[2].IsConnected)
            {
                ...
                dataCollector.AddLocalVariable(UniqueId, CurrentPrecisionType, WirePortDataType.FLOAT4, varName,
                    customScreenPosVal);
            }
            else
            {
                ...
                {
                    varName = GeneratorUtils.GenerateClipPositionOnFrag(ref dataCollector, UniqueId,
                ...
                else
                {
                    ...
                    {
                        varName = GeneratorUtils.GenerateScreenPositionNormalized(ref dataCollector, UniqueId,
            ...
            switch (m_selectedPatternInt)
            {
                default:
                case 0:
                    dataCollector.AddLocalVariable(UniqueId, CurrentPrecisionType, WirePortDataType.FLOAT2,
                        "clipScreen" + OutputId, surfInstruction);
                    functionResult = dataCollector.AddFunctions(m_functionHeader, m_functionBody,
                        "fmod(" + "clipScreen" + OutputId + ".x, 4)", "fmod(" + "clipScreen" + OutputId + ".y, 4)");
                    break;
                case 1:
                    dataCollector.AddLocalVariable(UniqueId, CurrentPrecisionType, WirePortDataType.FLOAT2,
                        "clipScreen" + OutputId, surfInstruction);
                    functionResult = dataCollector.AddFunctions(m_functionHeader, m_functionBody,
                        "fmod(" + "clipScreen" + OutputId + ".x, 8)", "fmod(" + "clipScreen" + OutputId + ".y, 8)");
                    break;
                case 2:
                {
                    if (!m_texPort.IsConnected)
                    {
                        ...
                        m_errorMessageTooltip =
                            "Please connect a texture object to the Pattern input port to generate a proper dithered pattern";
                        return "0";
                    }
                    else
                    {
                        ...
                        //GeneratePattern( ref dataCollector );
                        dataCollector.AddToUniforms(UniqueId, "float4 " + noiseTex + "_TexelSize;",
                            dataCollector.IsSRP);
...
#endif
                        {
                            ...
                            //else
                            functionResult = dataCollector.AddFunctions(m_functionHeader, m_functionBody, varName,
                                noiseTex + ", " + sampler, noiseTex + "_TexelSize");
                        }
                        else
                        {
                            functionResult = dataCollector.AddFunctions(m_functionHeader, m_functionBody, varName,
                                noiseTex, noiseTex + "_TexelSize");
                        }
                    }
                }
                    break;
            ...
            dataCollector.AddLocalVariable(UniqueId, CurrentPrecisionType, WirePortDataType.FLOAT, "dither" + OutputId,
                functionResult);
            ...
            if (m_inputPorts[0].IsConnected)
            {
                ...
                dataCollector.AddLocalVariable(UniqueId,
                    "dither" + OutputId + " = step( dither" + OutputId + ", " + driver + " );");
            }
