...
{
    [Serializable]
    [NodeAttributes("Camera Depth Fade", "Camera And Screen",
            ...
            bool ignoreLocalvar)
        {
            ...
            if (dataCollector.IsTemplate)
            {
                if (vertexPort.IsConnected)
                {
                    ...
                    dataCollector.TemplateDataCollectorInstance.RegisterCustomInterpolatedData(eyeDepth,
                        WirePortDataType.FLOAT, CurrentPrecisionType, eyeInstruction);
                }
                else
                {
                ...
                dataCollector.PortCategory == MasterNodePortCategory.Tessellation)
            {
                ...
                value = string.Format(CameraDepthFadeFormat, "-UnityObjectToViewPos( " + vertexVarName + " ).z", offset,
                    distance);
                RegisterLocalVariable(0, value, ref dataCollector, "cameraDepthFade" + OutputId);
                return m_outputPorts[0].LocalValue(dataCollector.PortCategory);
            }
            ...
            if (dataCollector.TesselationActive)
            {
                if (vertexPort.IsConnected)
                {
                    ...
                    RegisterLocalVariable(0, string.Format("-UnityObjectToViewPos( {0} ).z", vertexValue),
                        ref dataCollector, eyeDepth);
                }
                else
                {
                    eyeDepth = GeneratorUtils.GenerateScreenDepthOnFrag(ref dataCollector, UniqueId,
            ...
            else
            {
                if (vertexPort.IsConnected)
                {
                    ...
                    dataCollector.AddToVertexLocalVariables(UniqueId,
                        Constants.VertexShaderOutputStr + "." + varName + " = " + vertexInstruction + ";");
                ...
                else
                {
                    ...
                    dataCollector.AddToVertexLocalVariables(UniqueId,
                        Constants.VertexShaderOutputStr + ".eyeDepth = " + instruction + ";");
