...
{
    [Serializable]
    [NodeAttributes("Depth Fade", "Surface Data",
    ...
    public sealed class DepthFade : ParentNode
    {
        ...
        [SerializeField] private bool m_saturate = false;

        [SerializeField] private bool m_mirror = true;
            ...
            bool ignoreLocalvar)
        {
            ...
            if (vertexPosPort.IsConnected)
            {
                ...
                screenPosNorm = GeneratorUtils.GenerateScreenPositionNormalizedForValue(vertexPosVar, OutputId,
                    ref dataCollector, UniqueId, CurrentPrecisionType, !dataCollector.UsingCustomScreenPos);
            }
            else
            {
                if (dataCollector.IsTemplate)
                {
                    ...
                    if (!dataCollector.TemplateDataCollectorInstance.GetCustomInterpolatedData(
                            ...
                            PrecisionType.Float, ref ppsScreenPos, true, MasterNodePortCategory.Fragment))
                    {
            ...
            if (dataCollector.IsTemplate && dataCollector.IsSRP)
                finalVarValue = "( screenDepth" + OutputId + " - LinearEyeDepth( " + screenPosNorm +
                                ".z,_ZBufferParams ) ) / ( " + distance + " )";
            else
                finalVarValue = "( screenDepth" + OutputId + " - LinearEyeDepth( " + screenPosNorm + ".z ) ) / ( " +
                                distance + " )";
            ...
            dataCollector.AddLocalVariable(UniqueId, CurrentPrecisionType, WirePortDataType.FLOAT, finalVarName,
                finalVarValue);
