...
namespace AmplifyShaderEditor
{
    ...
    public sealed class ParallaxMappingNode : ParentNode
    {
        ...
        [SerializeField] private int m_selectedParallaxTypeInt = 0;

        [SerializeField] private ParallaxType m_selectedParallaxType = ParallaxType.Normal;
            ...
            bool ignoreLocalvar)
        {
            ...
            switch (m_selectedParallaxType)
            {
                ...
                case ParallaxType.Planar:
                    calculation = "( ( " + height + " - 1 ) * ( " + viewDirTan + ".xy / " + viewDirTan + ".z ) * " +
                                  scale + " ) + " + textcoords;
                    break;
            }

            dataCollector.AddLocalVariable(UniqueId, CurrentPrecisionType, m_outputPorts[0].DataType, localVarName,
                calculation);
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            m_selectedParallaxTypeInt =
                m_upperLeftWidget.DrawWidget(this, m_selectedParallaxTypeInt, m_parallaxTypeStr);
            if (EditorGUI.EndChangeCheck())
            {
                switch (m_selectedParallaxTypeInt)
                {
                    default:
                    case 0:
                        m_selectedParallaxType = ParallaxType.Normal;
                        break;
                    case 1:
                        m_selectedParallaxType = ParallaxType.Planar;
                        break;
                }
        ...
        void UpdateTitle()
        {
            m_additionalContent.text = string.Format(Constants.SubTitleTypeFormatStr,
                m_parallaxTypeStr[m_selectedParallaxTypeInt]);
        }
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            switch (m_selectedParallaxType)
            {
                default:
                case ParallaxType.Normal:
                    m_selectedParallaxTypeInt = 0;
                    break;
                case ParallaxType.Planar:
                    m_selectedParallaxTypeInt = 1;
                    break;
            }
